    ...
    <body>
       ...
       <form action="Principal" method="Post">
           Id: <input type="text" name="id" value="<%=item.getId() %>" size="2" pattern="[1-9 {1}[0-9]*" />
            <input type="" name="tipo" value="<%=item.getId() %>" required/>
            <br>
            Titulo:<input type="text" name="titulo" value="<%=item.getTitulo() %>" required/>
            <br>
            Autor:<input type="text" name="autor" value="<%=item.getAutor() %>" />
            <br>
             Estado:<input type="radio" name="Disponible" value="1" <%=(item.getEstado()==1) ? "checked":"" %> required/>Disponible
             <input type="radio" name="Prestado" value="2" <%=(item.getEstado()==2) ? "checked":"" %> required/>Prestado
             <br>
